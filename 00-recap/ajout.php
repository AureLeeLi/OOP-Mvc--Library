<?php
    ...
    $title = 'Ajouter un livre'; //titre de la page (onglet)
    ...
    if(!empty($_POST)) { //formulaire non vide
        ...
        if(empty($errors)){ //verif si le tableau d'erreurs contient qqch, si il est vide on fait la requête.
            ...
            addMessage('Le livre ' .$btitle. ' a bien été ajouté.');
...
?>
        ...
        <!-- verif si les erreurs passent bien avec le test -->
        <!-- var_dump($errors) -->
        <?php if(!empty($errors)) { ?>
        ...
        <form action="" method="post" class="w-1/2 mx-auto" enctype="multipart/form-data">
            ...
            <div class="text-center">
                <button class="bg-gray-900 px-4 py-2 text-white inline-block rounded hover:bg-gray-700 duration-200">Ajouter</button>
            </div>
        </form>
    </div>
